<template>
  <div class="pro-image-upload">
    ...
    <el-checkbox v-model="compress" v-if="!disabled">是否压缩</el-checkbox>
  </div>
</template>

<script setup lang="ts">
  import { blobToBase64, compressImage, getImageInfo, isArray } from '@daysnap/utils'
  import { useWithLoading } from '@/hooks'
  import { doImageUpload } from '@/api'
  ...
  const [loading, handleUpload] = useWithLoading(async (event: Event) => {
    ...
    const { imageType, modelValue, max } = props
    const upload = async (file: File) => {
      ...
      let base64 = (await blobToBase64(file)) as string
      if (compress.value) {
        const image = await getImageInfo(base64)
